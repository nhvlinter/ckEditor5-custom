...
export function insertImage( model, attributes = {}, insertPosition = null ) {
	model.change( writer => {
		const imageElement = writer.createElement( 'media', attributes );
...
export function getViewImgFromWidget( figureView ) {
	...
	return figureChildren.find( viewChild => viewChild.is( 'element', 'iframe' ) );
}
...
function isImageAllowedInParent( selection, schema, model ) {
	...
	return schema.checkChild( parent, 'media' );
}
...
function isInOtherImage( selection ) {
	return [ ...selection.focus.getAncestors() ].every( ancestor => !ancestor.is( 'element', 'media' ) );
}
